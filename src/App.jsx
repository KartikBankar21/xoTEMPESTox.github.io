  ...
  const handleAudioPlay = () => {
    ...
    if (currentAudio.current.paused) {
      currentAudio.current.play().then(() => {
        ...
        console.error("Playback failed (Autoplay blocked? Browser requires user interaction to start audio).", error);
...
useEffect(() => {
    if (currentAudio.current) {
        ...
        const handleLoadedMetadata = () => {
            ...
            console.log("Metadata loaded. Duration set:", currentAudio.current.duration);
        };
        ...
        currentAudio.current.addEventListener('canplaythrough', handleCanPlayThrough);
        ...
        return () => {
            ...
            currentAudio.current?.removeEventListener('canplaythrough', handleCanPlayThrough);
        };
    }
...
const handleMuteToggle = () => {
  ...
  if (isExpanded) {
    // Mobile/Tablet (default): Centered horizontally, 10rem from the bottom.
    positionClasses =
    ...
    // Desktop (lg:): Fixed left-center, narrow/tall vertical bar.
    positionClasses +=
      " lg:top-1/2 lg:-translate-y-1/2 lg:left-10 lg:w-20 lg:h-126";
  }
