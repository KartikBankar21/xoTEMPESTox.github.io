...
export const MiniPlayer = ({
  ...
  const formatText = (text, limit) => {
    if (isSmallScreen && text.length > limit) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden" data-theme={theme}>
      ...
      <div
        ...
        className="relative flex flex-row items-center justify-between lg:flex-col lg:rotate-180 h-full w-full pl-2 lg:p-2"
      >
        <div
          className={`w-9 h-9 lg:w-16 lg:h-16 rounded-lg lg:-rotate-90 shadow-md flex-shrink-0 overflow-hidden cursor-pointer active:scale-95 transition ${
            theme === "dark" ? "shadow-black" : "shadow-gray-200"
          }`}
          onClick={onExpand}
        >
          <img
        ...
        >
          <div
            className="w-full flex flex-col items-center justify-center lg:vertical-rl lg:-rotate-90 lg:space-y-1 group"
            style={verticalTextStyle}
          >
            <p
              className={`min-w-0 text-start mb-0! text-xs lg:text-xl font-bold tracking-wide truncate transition-colors ${
                theme === "dark"
                  ? "text-white opacity-80 group-hover:opacity-100"
                  : "text-black opacity-80 group-hover:opacity-100"
              }`}
            >
              {formatText(currentTrack.title, 14)}
            </p>
            <p
              className={`min-w-0 text-start mb-0! text-xs lg:text-lg truncate transition-colors ${
                theme === "dark" ? "text-white opacity-70" : "text-gray-600"
              }`}
            >
              {formatText(currentTrack.artist, 14)}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-row items-center justify-center lg:flex lg:flex-col lg:items-center lg:justify-between lg:rotate-180 space-x-2 lg:space-x-0 lg:space-y-4 flex-shrink-0">
          ...
          >
            ...
            <div className="relative z-10 transition-transform duration-300 group-hover:scale-110">
              {isPlaying ? (
                <Pause
                  className={`w-6 h-6 lg:w-8 lg:h-8 fill-current transition-colors duration-300 lg:rotate-90 ${
                    theme === "dark"
          ...
          >
            <div
              className={`absolute inset-0 rounded-full blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 scale-75 group-hover:scale-110 ${
                theme === "dark" ? "bg-white/10" : "bg-black/5"
              }`}
            />
            <div className="relative z-10 transition-transform duration-300 group-hover:scale-110">
              <SkipForward
                className={`w-5 h-5 lg:w-7 lg:h-7 fill-current rotate-0 lg:rotate-90 transition-colors duration-300 ${
                  theme === "dark"
                    ? "text-gray-400 group-hover:text-white"
                    : "text-gray-400 group-hover:text-black"
                }`}
