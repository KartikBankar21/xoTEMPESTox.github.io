...
import { 
  X, TrendingUp, Globe, Github
} from 'lucide-react';

const TechBadge = ({ icon: Icon, label }) => (
  <div className={`flex items-center bg-zinc-800 text-zinc-300 text-[11px] font-medium px-2 py-1 rounded-full border border-zinc-700`}>
    {Icon && <Icon size={12} className={`mr-1.5 text-zinc-400`} />}
    <span>{label}</span>
  </div>
);
      ...
      >
        ...
        <div className="p-6 space-y-0 max-h-[50vh] overflow-y-auto custom-scrollbar">
          <section className="p-4!">
            ...
            <p className="text-zinc-300 leading-relaxed text-[15px]">{project.description}</p>
          </section>

          <section className="p-4!">
            <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-500">Tech Stack</h2>
            <div className="flex flex-wrap gap-2">
              {project.techStack.map((tech) => (
                <TechBadge key={tech.name} label={tech.name} icon={tech.icon} />
              ))}
            </div>
          </section>

          <section className="p-4!">
            <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-500">Key Highlights</h2>
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {project.highlights.map((item, index) => (
                <li key={index} className="flex items-start space-x-3 p-3 rounded-lg bg-zinc-900/50 border border-zinc-800/50 group hover:border-zinc-600 transition-all">
