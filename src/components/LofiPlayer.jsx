  ...
  return (
    <div className="relative h-full w-full rounded-[2.5rem] overflow-hidden p-[1rem]">
      ...
      >
        {/* --- Menu Overlay --- */}
<div
  className={`menu-overlay z-20 rounded-[2.5rem] p-6 flex flex-col overflow-x-hidden transition-all duration-300 border ${
...
>
  <div className="flex justify-between items-center mb-6">
    <p className={`font-mono font-bold text-xl md:text-2xl uppercase tracking-widest flex items-center mb-0 transition-colors ${
    ...
    <button
      ...
      className={`p-2 rounded-full active:scale-95 transition-all shadow-md border ${
        theme === "dark"
  ...
  <ul className="flex-1 overflow-y-auto overflow-x-hidden space-y-2 pr-2 custom-scrollbar">
    {tracks.map((track, index) => {
      ...
      return (
        ...
        {/* Header */}
<div className={`w-full rounded-xl rounded-t-[2.5rem] px-4 py-2 mb-3 shadow-md border transition-colors duration-300 ${
  theme === "dark" 
  ...
  <div className="flex justify-between items-center relative">
    ...
    <h1 className={`font-mono text-lg md:text-xl uppercase tracking-widest text-center flex-1 truncate transition-colors ${
      theme === "dark" ? "text-white" : "text-black"
    }`}>
      {currentTrack.title}
    </h1>
    ...
    <button
      onClick={onCollapse}
      className={`p-1 rounded-full active:scale-95 transition-all shadow-inner border ${
        theme === "dark"
...
<div className={`w-full aspect-[3/4] rounded-xl overflow-hidden relative shadow-inner border-2 transition-colors duration-300 ${
  theme === "dark" 
  ...
  <div className="absolute bottom-0 left-0 right-0 p-4">
    ...
    <div className={`h-1.5 w-full rounded-full mb-2 relative group cursor-pointer transition-colors ${
      ...
      {/* Progress Handle */}
      <div
        className={`absolute top-1/2 -mt-1.5 w-3 h-3 rounded-full shadow-xl transition-all duration-100 ease-linear group-hover:scale-125 ${
    ...
    {/* Time Indicator */}
    <div className={`flex justify-between items-center text-lg transition-colors ${
      ...
      <span className="font-mono">
        ...
        {/* Controls Panel */}
        <div className={`w-full p-3 md:p-4 mt-3 rounded-xl rounded-b-[2.5rem] relative z-30 transition-colors duration-300 border shadow-md ${
  theme === "dark"
    ...
    : "bg-black/5 border-black/10 shadow-gray-200"
          ...
          <div className="flex justify-between items-center">
           <button
  onClick={onMenuToggle}
  className={`icon-btn text-xs md:text-xl font-mono px-2 py-1 md:px-4 md:py-2 shadow-inner font-bold rounded-lg transition-colors border ${
    theme === "dark"
...
>
  ...
  <button
    onClick={onPrev}
    className={`icon-btn p-1.5 md:p-2 rounded-full border transition-all active:scale-95 ${
      theme === "dark"
...
<div className="relative flex items-center" ref={volumeRef}>
  {showVolume && (
    <div 
      className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-16 backdrop-blur-sm p-3 pb-4 rounded-2xl shadow-2xl z-50 flex flex-col items-center transition-colors duration-300 border ${
        theme === "dark" 
    ...
    >
      <div className="relative h-32 w-full flex justify-center items-center">
        {/* Track Background */}
        <div className={`absolute w-1.5 h-full rounded-full overflow-hidden ${
          theme === "dark" ? "bg-white/10" : "bg-black/10"
        }`}>
          {/* Highlight Gradient - Adjusted 'to' color for light mode visibility */}
          <div
            className={`absolute bottom-0 w-full bg-gradient-to-t transition-all duration-75 ${
              theme === "dark" ? "from-purple-600 to-white" : "from-purple-600 to-purple-400"
            }`}
        ...
        {/* Slider Thumb */}
        <div
          className={`absolute left-1/2 -translate-x-1/2 w-4 h-4 rounded-full shadow-lg pointer-events-none transition-all duration-75 border-2 ${
            theme === "dark" 
      ...
      {/* Percentage Text */}
      <span className={`text-[10px] font-mono font-bold mt-3 tabular-nums ${
